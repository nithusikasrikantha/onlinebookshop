...
function getComments($conn) {
    ...
    while( $row =  $result->fetch_assoc()) {
        $id = $row['uid'];
        $sql2 = "SELECT * FROM comments.php WHERE id=' $id'";
...
function deleteComments($conn) {
   ...
   function getLogin($conn) {
    ...
    $sql = "SELECT * FROM comments WHERE uid='$uid' AND pwp='$pwp'";
    $result = $conn->query($sql);
    if (mysqli_num_rows($result) > 0) {
        if ($row =  $result->fetch_assoc()) {
         ...
         header("Location: ordering.php?loginsuccess");
         exit();
        }
       
    } else {
        header("Location: ordering.php?loginfailed");
        exit();
